...
function createOAuthBroker(ctx, hooks = {}) {
  ...
  const callbackServers = /* @__PURE__ */ new Set();
  const discoveryCache = /* @__PURE__ */ new Map();
  function callbackPage(res, status2, title, message) {
...
function apply3(ctx) {
  ...
  function run(argv) {
    return new Promise((resolve, reject) => {
      const child = spawn2(argv[0], argv.slice(1), { stdio: "ignore" });
  ...
  async function skillCatalog(sessionId) {
    ...
    const snapshot = await ctx.skills.snapshot(agent ? { scope: agent, cwd: agent.session.header.cwd } : {});
  ...
  function openWithSystem(abs) {
    return new Promise((resolve) => {
      ...
      const child = spawn2(cmd, [abs], { stdio: "ignore", detached: true });
      ...
      if (req.method === "POST") {
        ...
        switch (pathname) {
          ...
          case "/capabilities-api/mcp/oauth/login":
            out = await service.oauthLogin(body.name);
            break;
