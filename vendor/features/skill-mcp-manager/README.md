...
**Skill 管理**（`~/.dsh/skills`）
- 列出所有 skill（名称 / 描述 / 类型：目录 `SKILL.md` 或扁平 `.md` / 启用状态），支持按名称、描述搜索。Skill 列表使用独立接口，不等待 MCP OAuth 状态；加载失败或超时会结束加载状态并提供重试。
- **开关**：改写 frontmatter 的 `disable-model-invocation` 标记实现启用/禁用。
- **删除**：真实条目删除文件/目录；软链条目只删链接本身，不追目标。
- **导入**：通过 DSH 目录选择器选取根目录含 `SKILL.md` 的 Skill 文件夹，完整复制到 `~/.dsh/skills/<原目录名>`，保留脚本、资源、隐藏文件和空目录；源目录不变。已有目录或同名 Skill 会报错，不覆盖；复制失败会清理本次创建的目标目录。
...
- **OAuth MCP**：HTTP Server 可选择 OAuth；插件通过标准 protected-resource/authorization-server discovery、DCR 与 PKCE 直接打开系统浏览器，并用独立随机 `127.0.0.1` 端口接收 callback；凭据保存到 DSH `credentials` GrantRecord，不写入 `mcp.json`。
